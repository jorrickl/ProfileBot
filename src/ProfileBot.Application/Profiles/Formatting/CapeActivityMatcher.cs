...
{
    public class CapeActivityMatcher(string pattern, string output) : ActivityMatcher(pattern, output)
    {
        protected override bool OnMatchSuccess(Profile profile, Activity activity, Match match, out string? result)
        {
            ...
            var captureCapeName = match.Groups[1].Value;

            result = string.Format(Output, profile.Name, captureCapeName);
            return true;
        }
